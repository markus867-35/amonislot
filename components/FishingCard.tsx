// components/FishingCard.tsx
export default function FishingCard({ name, image }: { name: string, image: string }) {
  return (
    <div className="relative overflow-hidden rounded-xl border border-purple-800 bg-black group hover:border-yellow-500 transition-all">
      ...
      {/* Warna tombol khusus untuk Fishing */}
      <div className="absolute inset-x-0 bottom-0 bg-teal-900/90 py-2 text-center text-xs font-bold text-white opacity-0 group-hover:opacity-100 transition-opacity">
        MAINKAN SEKARANG
      </div>
    </div>
  );
