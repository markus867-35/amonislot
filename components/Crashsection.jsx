...
export default function CrashGamesSection() {
  ...
  const fetchCrashGames = async () => {
    try {
      setLoading(true);
      // Mengambil data khusus kategori CRASH dari database
      const { data, error } = await supabase
        .from('crash_games')
        .select('*')
        .ilike('category', 'CRASH')
        .order('id', { ascending: true });

      if (error) throw error;
      if (data) setCrashGames(data);
    } catch (error) {
      console.error('Gagal memuat crash games:', error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section id= "section-crash" className="w-full my-6 px-2 sm:px-0 bg-transparent md:bg-[#0b0e1b] text-black md:text-white">
      
      {/* CONTAINER UTAMA: Background transparan di mobile, ada background/border di desktop */}
      <div className="w-full max-w-7xl mx-auto rounded-3xl bg-transparent md:bg-[#0b0e1b] md:border md:border-cyan-950/85 md:shadow-2xl md:p-6 lg:p-8">
        ...
        {/* GRID CARD GAMES */}
        {loading ? (
          <div className="text-center py-10 text-gray-400 text-sm">Memuat crash games...</div>
        ) : crashGames.length === 0 ? (
          <div className="text-center py-10 text-gray-400 text-sm">Belum ada crash games tersedia.</div>
        ) : (
